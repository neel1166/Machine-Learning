...
while True:
	...
	for i in range(rows):
		if( train_lab.get(i)!=None):
			...
			sigmoid=train_lab.get(i) - (1 / (1 + (math.exp(-1 * wt))))
			for j in range(cols):
				derv_f[j] += sigmoid * data[i][j]
...
### Lodistic Discrimination evaluation
	log_dis=[]
	for i in range(rows):
		if(train_lab.get(i)!=None):
			temp = math.exp(-sum([a*b for a,b in zip(w,data[i])])) #computing e^-(wTx + w0)
			log_dis.append(train_lab.get(i)*math.log(1/(1+temp)) + (1-train_lab.get(i))*math.log(temp/(1+temp)))
	if(abs(prev-sum(log_dis)) <= stop_con):
		break
	prev=sum(log_dis)
